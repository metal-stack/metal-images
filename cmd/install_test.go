...
GRUB_SERIAL_COMMAND="serial --speed=115200 --unit=1 --word=8"`,
		},
		{
			...
			execMocks: []fakeexecparams{
				...
				{
					WantCmd:  []string{"efibootmgr", "-c", "-d", "/dev/sda1", "-p1", "-l", "\\\\EFI\\\\BOOT\\\\BOOTX64.EFI", "-L", "metal-ubuntu"},
					Output:   "",
					ExitCode: 0,
				},
				{
					WantCmd:  []string{"efibootmgr", "-c", "-d", "/dev/sdb1", "-p1", "-l", "\\\\EFI\\\\BOOT\\\\BOOTX64.EFI", "-L", "metal-ubuntu"},
					Output:   "",
					ExitCode: 0,
				},
				{
					WantCmd:  []string{"grub-install", "--target=x86_64-efi", "--efi-directory=/boot/efi", "--boot-directory=/boot", "--bootloader-id=metal-ubuntu", "--removable", "--no-nvram"},
					Output:   "",
					ExitCode: 0,
				},
				{
...
GRUB_SERIAL_COMMAND="serial --speed=115200 --unit=1 --word=8"`,
		},
		{
			...
			execMocks: []fakeexecparams{
				...
				{
					WantCmd:  []string{"grub2-install", "--target=x86_64-efi", "--efi-directory=/boot/efi", "--boot-directory=/boot", "--bootloader-id=centos", "--removable", "UUID=543eb7f8-98d4-d986-e669-824dbebe69e5"},
					Output:   "",
					ExitCode: 0,
				},
			},
...
GRUB_SERIAL_COMMAND="serial --speed=115200 --unit=1 --word=8"`,
		},
		{
			...
			execMocks: []fakeexecparams{
				...
				{
					WantCmd:  []string{"grub2-install", "--target=x86_64-efi", "--efi-directory=/boot/efi", "--boot-directory=/boot", "--bootloader-id=centos", "--removable", "--no-nvram", "UUID=ace079b5-06be-4429-bbf0-081ea4d7d0d9"},
					Output:   "",
					ExitCode: 0,
				},
				{
